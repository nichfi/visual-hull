...
folder_path = 'C:/Users/skippy/Downloads/VH_media/3_10oneplus(object_filmcan_photo)/'
...
def find_contours(imagename,debug_mode):
    ...
    ret, thresholded = cv.threshold(img, min(aruco_corner_darkness), 255,
...
for index, image_path in enumerate(image_paths):
    ...
    try:
        contour_points= (find_contours(image_path,debug_mode)[::10]) 
    except:
    ...
    print()
    #optional  
    if len(projection_list) >  4: #test
        break


for index, value in enumerate (projection_list) :
    value = msh.model.occ.intersect([(3,1)],[(3,value+1)],removeObject= True,
                                    removeTool = True)
